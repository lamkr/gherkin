...
void main()
{
  ...
  test('Provides emoji dialect', () {
    var em = GherkinDialectProvider(languages).getDialect('em');
  ...
  test('Provides language list', () {
    var gherkinLanguages = GherkinDialectProvider(languages).languages;
    expect(gherkinLanguages.contains('en'), isTrue);
  });

  test('Provides native name which is used in cucumber jvm code generation',(){
    var no = GherkinDialectProvider(languages).getDialect('no');
    expect(no.nativeName, 'norsk');
  });

  test('Should throw NoSuchLanguageException for invalid language', () {
    var provider = GherkinDialectProvider(languages);
  ...
  test('Should throw NoSuchLanguageException for invalid default language', () {
    var provider = GherkinDialectProvider(languages, 'nosuchlang');
